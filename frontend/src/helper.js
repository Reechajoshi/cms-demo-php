...
CHelp = {
	...
	displaySubMenu: function( anchor_tag )
	{
		// var parent_div = $(anchor_tag).eq(0).find( 'div.submenu' );
		var parent_div = CUtil.getParentByName( anchor_tag, "main_menu_div" );
		// var submenu_div = parent_div.getElementsByTagName( "div" );
		var submenu_div = CUtil.getChildByName(parent_div, 'submenu', 'DIV', false);
		var expand_sign = CUtil.getChildByName(anchor_tag,'ipmid','SPAN',false);
		// submenu_div.style.display = "block";
		// submenu_div.style.display = "block";
		if( submenu_div.style.display == "block" )
		{
			submenu_div.style.display = "none";
			expand_sign.innerHTML = "+";
		}
		else
		{
			submenu_div.style.display = "block";
	...
	downloadMe : function(did){
	
		CTalk.sendSimplePost( "download.x?ac=enq",function( resp ){ 
