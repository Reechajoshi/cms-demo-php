...
*/
	...
	if( $allcnt > 0 )
	{
		...
		if( $cnt > 0 )
		{
			$q = "select ename, email,econt,enote,eflag,edate from enquiry where ename like '%$srctxt%' order by edate desc";
				...
				while( ( $row = $hlp->_db->db_get( $res ) ) !== false )
				{
					...
					echo( '<div style="padding-top:7px;" >
							<table class=txt style="width:650px;" >
								<tr>	
									<td style="'.$caption_style.'" ><b>Category:</b></td><td style="'.$val_style.'" >'.$ename.'&#160;</td>
								</tr>
								<tr>	
									<td style="'.$caption_style.'" ><b>Category:</b></td><td style="'.$val_style.'" >'.$edate.'&#160;</td>
								</tr>
							</table>		
					...
					echo( $hlp->getLinkAncHtml( 'aeditm',60,'asb ','#','tt="View :'.$ename.'";addDynTabEx("'.$parent_tab.'","View ","'.$pname.'",tt,"'.$me.'?b=enqview&pid='.base64_encode( $pid ).'",true);',20,'images/ic/itick.gif','View',$parent_tab ) );
					echo( $hlp->getLinkAncHtml( 'aeditm',60,'asb ','#','tt="View :'.$ename.'";addDynTabEx("'.$parent_tab.'","View ","'.$pname.'",tt,"'.$me.'?b=enqview&pid='.base64_encode( $pid ).'",true);',20,'images/ic/itick.gif','View',$parent_tab ) );
