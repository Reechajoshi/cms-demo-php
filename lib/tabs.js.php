...
/*		CTabs._M_IMG_TABL.src = "images/tabl.gif";
	...
	tabs: function(tabconid, params)
	{
		...
		if(this._def.userTabControl_WithAccel)
		{
			var me = this;
			
			CUi.addShiftKeyHandler( 'C', function() {				
				if( (me._curSelTabI > 0) && (me._tabs[me._curSelTabI].userTabControl) )
					me.closeTabByIX( me._curSelTabI );
			}, true );		

			CUi.addShiftKeyHandler( 'R', function() {
				me.refreshCurrentTab_Force();
			}, true );		

			CUi.addShiftKeyHandler( CUtil.M_CHAR_CODE_LEFT_ARROW, function() {				
				me.selectPrev();
			}, true );					
			
			CUi.addShiftKeyHandler( CUtil.M_CHAR_CODE_RIGHT_ARROW, function() {				
				me.selectNext();
			}, true );
		}
	},	
	...
	tabViewScrollAdjustForCB: function(tabViewOB)	
	{
		var tWOB = CUtil.getChildByName(tabViewOB,'tabViewWrapBox','DIV',true);
		if(tWOB)
		{
			tWOB.style.position = 'relative';
			tWOB.style.left = '0px';
			tWOB.style.top = '0px'
			tWOB.style.width = tabViewOB.scrollWidth;
			tWOB.style.height = tabViewOB.scrollHeight;
		}
	},
	
	htmlWrap: function(inhtml)
	{
		return( "<div name=tabViewWrapBox style='position:relative;top:0px;left:0px;width:100%;height:100%;background-color: " + '#fff' + ";'>" + inhtml + "</div>" );
	}
};
...
CTabs.prototype = CTabs.tabs.prototype = {
	...
	// FUNCTION TO HANDLE ALL REMOTE CALLS FOR DYNAMIC CALLBACK TABS
	requestRemoteCallTabData: function(curTabIXForThisReq, furl, params_dname, params_dval, scriptUrl, cb )
	{
		var me = this;
		
		CTalk.sendPost( furl, params_dname, params_dval, function(resp) {
			if(CUtil.varok(resp) && resp != "")
			{
				try {
					var funcSetHTML = function(html)
					{
						try { 		
							var tab = me._tabs[ curTabIXForThisReq ];
							while((!CUtil.varok(tab)) && (curTabIXForThisReq>0))
								{ tab = me._tabs[ --curTabIXForThisReq ]; }
							
							html = CTabs.htmlWrap( html );
							
							// SET THE LAST HTML RETREIVED AND THE DATA NEEDED TO RETRIEVE IT
							tab.lastCallData = new Array( furl, params_dname, params_dval );							
							me._tabs[ curTabIXForThisReq ] = tab;
							
							me.setTabViewHTML(tab, html, (curTabIXForThisReq == me._curSelTabI));
							
							if(CUtil.isfunc(cb)) { cb(true, html); }
						} catch(e) { }
					};
					
					if(CUtil.varok(scriptUrl))
						CUtil.dynSourceScrip( scriptUrl, function() { funcSetHTML(resp); } );
					else
						funcSetHTML(resp);
				} catch(e) {  }
			}
			...
			{
				if(CUtil.isfunc(cb)) { cb(false); }
			}
		} );
		
		return( me.setTabViewHTML( me._tabs[ curTabIXForThisReq ], "working, please wait ..." ) );
	},
	
	submitFormData: function(furl)
	{	
		this.submitPostFormEx(null, null, furl);
	},
	
	submitPostFormEx: function(fchob, fname, furl, scriptUrl)
	{
		var params_dname = new Array(), params_dval = new Array();
				
		if( CUtil.varok(fchob) && CUtil.varok(fname) )
		{
			var fob = CUtil.getParentByName(fchob,fname);
			
			if(fob)
			{
				CUtil.applyToMultiChildNodes(fob, [ 'INPUT','SELECT' ], true, function(ob) {
					params_dname.push(ob.getAttribute('name'));
					if( ob.type == 'checkbox' )
					{	
						if( ob.checked )
							params_dval.push("1");	
						...
						params_dval.push(ob.value);	
				} );
			}
		}
		
		fob.innerHTML = (this.requestRemoteCallTabData( this._curSelTabI, furl, params_dname, params_dval, scriptUrl, function(bok, resp) { 
			if(!bok) { fob.innerHTML = "Something went wrong; please try again later ..."; }
		} ) );
	},
	
	submitPostFormDirect: function(furl, params_dname, params_dval)
	{
		return( this.requestRemoteCallTabData( this._curSelTabI, furl, params_dname, params_dval ) );
	},
	...
	{
		return( this._tabs[this._curSelTabI] );
	},
	
	setCurrentTab: function(tab, curTabIX)
	{
		if(!CUtil.varok(curTabIX))
			curTabIX = this._curSelTabI;
			
		this._tabs[curTabIX] = tab;
	},
	
	setTabViewHTML: function(tab, html, bdispOnTabView)
	{		
		if(!CUtil.varok(bdispOnTabView))
			bdispOnTabView = true;
			
		var vel = this.getTabViewElm();
			
		tab._lastHTML = CUtil.cutBodyHTML( html );							
		
		if(bdispOnTabView == true)
			vel.innerHTML = ( tab._lastHTML );
			
		return(tab._lastHTML);
	},
	...
	getTabParams: function(ttype,tid, p)	
	{
		var tp = {
			...
			cbProcessInputCodes: true, // IF SET THEN ON A TAB SHIFT FOCUS AND A RECURSE OVER REM TAGS, EVENTS SUCH AS ONCHANGE WILL BE PROCESSED
			callbackFunc: function(me,tabx) {},
			callbackParam: null
		};
	...
	addCallBackURLTab: function(tabid,params)
	{	
		params.tabViewScrollAdjustOn = true;
		params.callbackFunc = function(_tabo, _tab) {
			var tabo = _tabo; //_realId
			var tab = _tab;
			
			if(CUtil.varok( _tab._lastHTML ) && _tab._lastHTML != false )
			{				
				if( CUtil.varok(_tab.lastInputMapHTML) )
				{
					setTimeout( function() {
						try {
							var vel = _tabo.getTabViewElm();						
							_tabo.recurseRemTags(vel, function(_tabo,ob) { 
								var vali = tab.lastInputMapHTML[ob.getAttribute('name')]							
								if(typeof vali != "undefined")
								{
									if( ob.tagName == "SELECT" )
									{
										...
										if(tab.cbProcessInputCodes)
										{
											if(CUtil.isfunc(ob.onchange) )
												ob.onchange();
										}
									}
				...
				return(_tab._lastHTML);
			}
			else
			{
				return( tabo.requestRemoteCallTabData( tab._tix, tab.cbURL ) );				
			}
		};
		
		return( this.addCallBackTab( tabid, params ) );
	},
	...
	// close a tab done by its zero-based index
	closeTabByIX: function(tabix)
	{
		if(tabix==0) //safty
			alert('Cannot remove this TAB');
		else
		{
			if(this.checkSafeRemove(tabix))
			{
				this.clearTabViewOf(tabix);
				this.removeTabByIndex(tabix);
				this.selectPrev();
			}
		}
	},
	...
	selectNext: function()
	{
		if((++this._curSelTabI) >= this.tabCount())
		{
...
/*
	...
	*/
	...
	{		
		...
		html += "<div class=tabview id='" + this._realId + "tabView' name=tabView " + ( (tab.tabViewScrollAdjustOn == true) ? ("onScroll='CTabs.tabViewScrollAdjustForCB(this);'") : ("") ) + " style='" + bgStyle + ("border:" + this._def.borderUnSelect) + ";height:"+(rh - ((this._def.topIncluded)?(this._def.areaTopMargin):(0))) + "'>";
	...
	{		
		...
		if(CUtil.varok(this._curSelTabI) && this._curSelTabI!=si) 
		{
			...
			if(this._tabs[this._curSelTabI].type == CTabs._M_TAB_TYPE_CALLBACK)
			{ 					
				var me = this;
				me._tabs[me._curSelTabI].lastInputMapHTML = new Array();					
				this.recurseRemTags(vel, function(me,ob) { 
					if(ob.type=="checkbox")
						me._tabs[me._curSelTabI].lastInputMapHTML[ob.getAttribute('name')] = ob.checked; 
					else
						me._tabs[me._curSelTabI].lastInputMapHTML[ob.getAttribute('name')] = ob.value; 
				} );		
			}
		}
	...
	getTabViewHTML: function(tab) 
	{
		...
		{			
			return( tab.callbackFunc(this,tab) );			
		}
	...
	setTabView: function(vel,si) 
	{
		...
		{			
			...
			vel.innerHTML = (this._tabs[si].callbackFunc(this, this._tabs[si]));		
		else if(this._tabs[si].type == CTabs._M_TAB_TYPE_IFRAME)
		{
			vel.innerHTML = CTabs.htmlWrap( this.setTabIFrame(vel,this._tabs[si]) );
		}
	},
	...
	getIFrameObject: function()
	{
		...
		{
			var vel = this.getTabViewElm();
			var tf = CUtil.getChildByName(vel,tab.id,'DIV',true);
			if(tf)
	...
	refreshCurrentTab_Force: function()
	{
		if( this._tabs[ this._curSelTabI ].type == CTabs._M_TAB_TYPE_CALLBACK )
		{
			this._tabs[ this._curSelTabI ]._lastHTML = false;
			this._tabs[ this._curSelTabI ].callbackFunc(this, this._tabs[this._curSelTabI]);
		}
		else if( this._tabs[ this._curSelTabI ].type == CTabs._M_TAB_TYPE_IFRAME )
		{
